...
def betweenness_centrality(
    G, k=None, normalized=True, weight=None, endpoints=False, seed=None, threshold=None,
):
    ...
    threshold : integer or float. Defines the maximum length of a shortest path.
    Returns
    -------
    nodes : dictionary
    ...
    """
    assert threshold > 0, f"threshold greater than 0 expected, got: {threshold}"
    ...
    for s in nodes:
        ...
        if weight is None:  # use BFS
            S, P, sigma, _ = _single_source_shortest_path_local(G, s, threshold=threshold)
...
@py_random_state(4)
def edge_betweenness_centrality(G, k=None, normalized=True, weight=None, seed=None, threshold=None):
...
def _single_source_shortest_path_local(G, s, threshold=None):
    ...
    while Q:  # use BFS to find the shortest paths
        v = Q.popleft()
        S.append(v)
        Dv = D[v]
        sigmav = sigma[v]
        for w in G[v]:  # for each neighbor
            if (Dv + 1) <= threshold:  ### experimental!!!
                if w not in D:  # if not explored yet
                    Q.append(w)  # append to the queue
                    D[w] = Dv + 1
